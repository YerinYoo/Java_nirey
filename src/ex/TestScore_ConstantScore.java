...
public class TestScore_ConstantScore {
	...
	public static void main(String args[]){
		...
		int[][] score={
						{96,92,98}, // 점수 초기화(2차원 배열: byte형)
						{88,90,68},
						{98,80,75},
};
		
		//저장 공간 생성
		short[] sum = new short[3]; // 합계 저장 공간
		short[] sum_subject = new short[3]; // 과목별 합계 저장 공간
		short total_sum = 0; // 전체 합계 저장 공간
		...
		short[] rank = new short[3]; // 개인별 석차가 저장될 공간
		...
		for(int i=0; i<score.length; i++){
			...
			rank[i]=(short)stat; // stat에서 계산된 석차가 rank의 i번째 요소에 대입
		}
		...
		System.out.println("===================================================");
		System.out.println("타입\t국어\t영어\t수학");
		//합계 표시
		System.out.print("합계\t");
		for(int i=0; i<score[1].length; i++){
